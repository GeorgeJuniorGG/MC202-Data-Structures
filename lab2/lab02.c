...
int main() {
    ...
    if(atendentes == 1){ //resolve o caso de ter 1 atendente
        ...
        for(i = atendentes; i < clientes; i++){
            ...
            mat[j][2] = mat[i][1] + mat[i][0]; //a 3a coluna guarda o tempo em que o cliente sai
            ordem[0] = mat[i][1] + mat[i][0]; //o menor momento de saida eh substituido pelo momento de saida do proximo cliente
            for(k = 1; k < (atendentes + 1); k++){ //determina qual caixa vai esvaziar primeiro
            //AINDA PRECISA CONSERTAR, SO FUNCIONA SE TIVER 2 ATENDENTES
                if(mat[0][2] < mat[k - 1][2]){
                    ordem[k - 1] = mat[0][2];
                    if (k > 0){
                        j = 0;
                    }    
